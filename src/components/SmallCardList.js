...
function SmallCardList() {
    ...
    const allBoxLink = allLink.map((el)=>(
        ...
        <li className="styleTT__boxEL" style={{backgroundImage : `url(${el.img})`}}>
        <Link to={el.link} aria-label="direction vers un style de tattoo" className="sect__txt styleTT__link">
           <span> {el.name}</span>
        </Link>
    </li>
